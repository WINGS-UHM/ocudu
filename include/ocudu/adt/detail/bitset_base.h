...
/// \brief Base class of the ocudu bitsets holding the operations that depend solely on the bit index order
/// (\c LowestInfoBitIsMSB), but neither on the bitset capacity nor on its current size.
///
/// \tparam LowestInfoBitIsMSB Bit index order in memory. See \c bounded_bitset for more details.
template <bool LowestInfoBitIsMSB>
...
protected:
  ...
  OCUDU_FORCE_INLINE static constexpr size_t nof_words_(size_t size) noexcept
  {
    return divide_ceil(size, bits_per_word);
  }

  static constexpr void assert_within_bounds_(size_t pos, size_t size, bool strict) noexcept
  {
  ...
  static constexpr void assert_range_bounds_(size_t startpos, size_t endpos, size_t size) noexcept
  {
  ...
  static auto make_word_mask_range(size_t start, size_t stop) noexcept
  {
...
class bitset_crtp : public bounded_bitset_detail::base_bounded_bitset<LowestInfoBitIsMSB>
{
...
public:
  ...
  int find_lowest(size_t startpos, size_t endpos, bool value = true) const noexcept
  {
    for (const auto& [word_idx, mask] : make_word_mask_range(startpos, endpos)) {
      ...
      if (w != 0) {
        ...
        if constexpr (LowestInfoBitIsMSB) {
          pos += base_t::convert_bitpos_(find_first_msb_one(w));
        } else {
  ...
  int find_highest(size_t startpos, size_t endpos, bool value = true) const noexcept
  {
    ...
    for (size_t i = lastword; i != startword - 1; --i) {
      ...
      if (w != 0) {
        if constexpr (LowestInfoBitIsMSB) {
          return static_cast<int>(i * bits_per_word + base_t::convert_bitpos_(find_first_lsb_one(w)));
        } else {
...
protected:
  ...
  OCUDU_FORCE_INLINE constexpr bool test_(size_t bitpos) const noexcept
  {
    ...
    const word_t bitmask = base_t::maskbit(bitpos);
  ...
  constexpr void set_(size_t bitpos) noexcept
  {
    const size_t word_idx = bitpos / bits_per_word;
    ocudu_assume(word_idx < buffer.size());
    buffer[word_idx] |= base_t::maskbit(bitpos);
  }
