...
public:
  explicit ul_dci_request_builder(ul_dci_request& msg_) : msg(msg_) { msg.num_pdus_of_each_type.fill(0); }
  ...
  ul_dci_request_builder& set_basic_parameters(uint16_t sfn, uint16_t slot)
  {
    msg.sfn  = sfn;
    msg.slot = slot;
  ...
  dl_pdcch_pdu_builder add_pdcch_pdu(unsigned nof_dci_in_pdu)
  {
    ...
    msg.num_pdus_of_each_type[ul_dci_request::DCI_INDEX] += nof_dci_in_pdu;
