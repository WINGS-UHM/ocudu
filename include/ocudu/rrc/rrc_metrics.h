...
struct rrc_connection_counter_with_cause {
  ...
  void increase(establishment_resume_cause_t cause)
  {
    if (static_cast<unsigned>(cause) < counters_by_cause.size()) {
      counters_by_cause[static_cast<unsigned>(cause)]++;
    } else {
      counters_by_cause[static_cast<unsigned>(establishment_resume_cause_t::unknown)]++;
    }
  }

  unsigned size() const { return counters_by_cause.size(); }

  unsigned get_count(establishment_resume_cause_t cause) const
  {
    return counters_by_cause[static_cast<unsigned>(cause)];
  }

  unsigned get_count(unsigned index) const { return counters_by_cause[index]; }

  establishment_resume_cause_t get_cause(unsigned index) const { return establishment_resume_cause_t(index); }
