...
template struct asn1::protocol_ie_single_container_s<tx_comb_ext_ies_o>;
...
const char* pos_sib_type_opts::to_string() const
{
  ...
  return convert_enum_idx(names, 47, value, "pos_sib_type_e");
}
...
const char* bw_srs_c::fr2_opts::to_string() const
{
  ...
  return convert_enum_idx(names, 7, value, "bw_srs_c::fr2_e_");
}
uint16_t bw_srs_c::fr2_opts::to_number() const
{
  static const uint16_t numbers[] = {50, 100, 200, 400, 800, 1600, 2000};
  return map_enum_number(numbers, 7, value, "bw_srs_c::fr2_e_");
}
...
OCUDUASN_CODE ul_rtoameas_ext_ies_o::ext_c::pack(bit_ref& bref) const
{
  varlength_field_pack_guard varlen_scope(bref, true);
  switch (type_) {
    case types::extended_add_path_list:
...
OCUDUASN_CODE ul_rtoameas_ext_ies_o::ext_c::unpack(cbit_ref& bref)
{
  varlength_field_unpack_guard varlen_scope(bref, true);
  switch (type_) {
    case types::extended_add_path_list:
...
OCUDUASN_CODE ul_rtoameas_ext_ies_container::pack(bit_ref& bref) const
{
  uint32_t nof_ies = 0;
  nof_ies += extended_add_path_list_present ? 1 : 0;
...
OCUDUASN_CODE ul_rtoameas_ext_ies_container::unpack(cbit_ref& bref)
{
  uint32_t nof_ies = 0;
  unpack_length(nof_ies, bref, 1u, 65535u, true);
...
OCUDUASN_CODE ul_rtoameas_s::pack(bit_ref& bref) const
{
  bref.pack(ext, 1);
  HANDLE_CODE(bref.pack(add_path_list.size() > 0, 1));
  HANDLE_CODE(bref.pack(ie_exts_present, 1));
...
OCUDUASN_CODE ul_rtoameas_s::unpack(cbit_ref& bref)
{
  bref.unpack(ext, 1);
  bool add_path_list_present;
  HANDLE_CODE(bref.unpack(add_path_list_present, 1));
...
OCUDUASN_CODE srscfg_s::pack(bit_ref& bref) const
{
  bref.pack(ext, 1);
  HANDLE_CODE(bref.pack(ie_exts_present, 1));
...
OCUDUASN_CODE srscfg_s::unpack(cbit_ref& bref)
{
  bref.unpack(ext, 1);
  HANDLE_CODE(bref.unpack(ie_exts_present, 1));
