...
TEST_P(du_high_many_cells_tester, when_cell_stopped_then_ues_are_released)
{
  ...
  const unsigned rem_cell_idx = test_random::uniform_int<unsigned>(0, GetParam().nof_cells - 1);
...
TEST_P(du_high_many_cells_tester, when_cell_restarted_then_ues_can_be_created)
{
  ...
  const unsigned nof_slots_before_reactivation = test_random::uniform_int<unsigned>(0, 500);
