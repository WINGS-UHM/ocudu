...
protected:
  test_srs_res_list_builder()
  {
    ...
    auto& srs_cfg                     = du_cell_cfg.init_bwp_builder.srs_cfg;
    ...
    nof_symbols_srs_area = du_cell_cfg.init_bwp_builder.srs_cfg.max_nof_symbols.value();
    ...
    nof_symbol_intervals =
        nof_symbols_srs_area / static_cast<unsigned>(du_cell_cfg.init_bwp_builder.srs_cfg.nof_symbols);
  }

  unsigned compute_expected_srs_list_size() const
  {
    return nof_symbol_intervals * static_cast<unsigned>(du_cell_cfg.init_bwp_builder.srs_cfg.tx_comb) *
           static_cast<unsigned>(du_cell_cfg.init_bwp_builder.srs_cfg.cyclic_shift_reuse_factor) *
           static_cast<unsigned>(du_cell_cfg.init_bwp_builder.srs_cfg.sequence_id_reuse_factor);
  }
  ...
  bool check_res_list_elements(span<const du_srs_resource> srs_res_list) const
  {
    ...
    for (const auto& srs_res : srs_res_list) {
      if (srs_res.tx_comb_offset >= static_cast<unsigned>(du_cell_cfg.init_bwp_builder.srs_cfg.tx_comb)) {
        return false;
      }
    ...
    const std::vector<unsigned> tx_comb_offsets =
        du_cell_cfg.init_bwp_builder.srs_cfg.tx_comb == ocudu::tx_comb_size::n2 ? std::vector<unsigned>{0U, 1U}
    ...
    for (unsigned sym_start =
      ...
      const auto nof_elements = static_cast<unsigned>(std::count_if(
          srs_res_list.begin(),
          srs_res_list.end(),
          [sym_start, srs_sym_length = du_cell_cfg.init_bwp_builder.srs_cfg.nof_symbols](const du_srs_resource& res) {
