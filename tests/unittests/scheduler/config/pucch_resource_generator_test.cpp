...
TEST_P(test_ue_pucch_config_builder, test_validator_too_many_resources)
{
  ...
  const auto harq_idx_cfg = test_random::uniform_int<unsigned>(0, nof_harq_cfg_per_ue - 1);
  const auto sr_idx_cfg   = test_random::uniform_int<unsigned>(0, nof_sr_res_per_cell - 1);
  const auto csi_idx_cfg  = test_random::uniform_int<unsigned>(0, nof_csi_res_per_cell - 1);
