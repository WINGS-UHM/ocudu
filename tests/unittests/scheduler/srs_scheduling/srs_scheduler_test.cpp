                                             ...
                                             const std::optional<tdd_ul_dl_config_common>& tdd_cfg)
{
  ...
  if (tdd_cfg.has_value()) {
    ...
    srs_offset = test_random::uniform_int<uint16_t>(0, static_cast<uint16_t>(srs_period) - 1);
...
TEST_P(srs_scheduler_tester, test_different_periods)
{
  ...
  const auto add_ue_slot = test_random::uniform_int<unsigned>(0, res_grid.ring_size());
